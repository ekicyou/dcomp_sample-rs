...
- [x] **タスク1.6: ECS統合** *(2025-11-11追加)*
  - Bevy ECSを使用したウィンドウ管理システムの実装
  - `Window`コンポーネントでウィンドウを宣言的に作成
  - シングルスレッド実行でメインスレッドでのウィンドウ作成を保証

🏆小さな報酬: 空っぽのウィンドウが画面に表示され、ドラッグしたり、サイズ変更したり、閉じたりできるようになります。あなたのアプリケーションがはじめて姿を現す瞬間です！

**達成状況**: ✅ `simple_window`サンプルで動作確認済み。ECS方式でウィンドウを宣言的に作成・表示できます。
