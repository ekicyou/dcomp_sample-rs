...
- [x] 1. WIC ピクセル取得APIの追加
- [x] 1.1 (P) IWICBitmapSource拡張トレイトを実装する
...
- [x] 2. AlphaMaskデータ構造の実装
- [x] 2.1 (P) αマスク構造体を実装する
...
- [x] 2.2 (P) BitmapSourceResourceにαマスクフィールドを追加する
...
- [x] 3. HitTestMode拡張とAPIの追加
- [x] 3.1 (P) HitTestModeにAlphaMaskバリアントを追加する
  - ヒットテストモード列挙型にαマスク判定用のバリアントを追加
  - 既存のNone/Boundsバリアントの動作を維持
  - _Requirements: 1_

- [x] 3.2 (P) HitTestコンポーネントにalpha_mask()コンストラクタを追加する
  - αマスクモードでHitTestを生成するコンストラクタメソッドを追加
  - 既存のnone()/bounds()メソッドと同じパターンで実装
  - _Requirements: 1, 6_

- [x] 4. αマスク非同期生成システムの実装
- [x] 4.1 αマスク生成システムを実装する
...
- [x] 4.2 αマスク生成システムをスケジュールに登録する
  - WidgetGraphicsスケジュールに新システムを追加
  - draw_bitmap_sourcesシステムの直後に実行されるよう順序を指定
  - _Requirements: 3_

- [x] 5. hit_test_entity関数のAlphaMask対応
- [x] 5.1 hit_test_entityにαマスク判定分岐を追加する
...
- [x] 6.3* デモアプリケーションでの動作確認用コードを追加する
  - taffy_flex_demoのRedBoxの色トグル機能を追加
  - 画像の透明部分クリック時に背景(RedBox)の色が変わることを視覚的に確認可能
  - _Requirements: 1, 6_
