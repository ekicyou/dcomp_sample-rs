...
- [x] 1. DpiChangeContextスレッドローカル基盤の実装
- [x] 1.1 (P) DpiChangeContext構造体とスレッドローカルストレージを定義する
...
- [x] 1.2 (P) SetWindowPosCommandキュー基盤を実装する
...
- [x] 1.3 (P) WindowPosChanged ECSコンポーネントを定義する
  - bool型の単純なコンポーネントを定義
  - SparseSetストレージ属性を指定
...
- [x] 3. WM_WINDOWPOSCHANGED処理をWorld借用区切り方式に改修する
- [x] 3.1 第1借用セクションを実装する
...
- [x] 4. apply_window_pos_changesシステムを修正する
...
- [x] 6. WM_DPICHANGED_DEFERRED関連コードを削除する
  - カスタムメッセージ定義は互換性のため保持（ハンドラは非推奨メッセージとして処理）
