...
- [x] 2.2 (P) AnimationVariableDef enum 定義
...
- [x] 2.5* (P) Unit tests — DolaDocument/AnimationVariableDef/DynamicValue serde round-trip
...
- [x] 3.3 (P) ParametricEasing enum 定義
...
- [x] 3.4* (P) Unit tests — EasingFunction/EasingName/ParametricEasing serde round-trip
...
- [x] 4.1 (P) TransitionValue enum 定義
...
- [x] 4.3 (P) TransitionRef enum 定義
  - `#[serde(untagged)]` + 2バリアント: Named(String), Inline(TransitionDef)
  - 文字列 → Named、オブジェクト → Inline の順で試行（Req 2.8, 2.9）
- [x] 4.4* (P) Unit tests — TransitionValue/TransitionDef/TransitionRef serde round-trip
...
- [x] 5.1 (P) InterruptionPolicy enum 定義
...
- [x] 5.6 (P) BetweenKeyframes 構造体定義
  - `from: String, to: String`（Req 4.4）
- [x] 5.7* (P) Unit tests — Storyboard/StoryboardEntry/KeyframeRef serde round-trip
...
- [x] 6.1 (P) PlaybackState enum 定義
  - 5バリアント: Idle, Playing, Paused, Completed, Cancelled（Req 6.1）
  - `#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]`
- [x] 6.2 (P) ScheduleRequest 構造体定義
  - `storyboard: String, start_time: f64`（Req 6.3, 6.5）
  - `#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]`
- [x] 6.3* (P) Unit tests — PlaybackState/ScheduleRequest serde round-trip
...
- [x] 7.5 V6: キーフレーム参照検証（前方参照許可 + 暗黙的KF追跡）
...
- [x] 7.6* Unit tests — V1-V6バリデーション正常系・異常系
...
- [x] 8.6* (P) Unit tests — V7-V13バリデーション正常系・異常系
...
- [x] 9.2* Unit tests — DolaDocumentBuilder
...
- [x] 10.1 StoryboardBuilder 実装
  ...
  - メソッドチェーン可能な move semantics
- [x] 10.2* Unit tests — StoryboardBuilder
...
- [x] 11.2* (P) Integration tests — JSON round-trip
...
- [x] 11.3* (P) Integration tests — TOML round-trip（feature `toml`）
  - 同じDolaDocumentのTOML round-trip（Req 5.2）
  - TOML配列インライン vs 改行挙動確認
  - BTreeMap キー順序の決定性確認
- [x] 11.4* (P) Integration tests — YAML round-trip（feature `yaml`）
