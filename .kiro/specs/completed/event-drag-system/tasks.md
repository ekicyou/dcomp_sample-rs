...
### ✅ **DPIスケール問題：解決済み（2026-02-12）**

2025-12-08 に発見された1.25倍速バグは、別仕様 `dpi-coordinate-transform-survey` およびその子仕様群で**解決済み**：

- **wintf-fix1** (`sync-window-pos-consolidation`): `sync_window_pos` と `update_window_pos_system` を `window_pos_sync_system` に統合。重複パイプライン解消。
- **wintf-fix3** (`sync-arrangement-enable`): `sync_window_arrangement_from_window_pos` を有効化。Win32→ECS逆同期。
- **wintf-fix4** (`feedback-loop-simplify`): TLSフラグ `IS_SELF_INITIATED` + `guarded_set_window_pos()` によるエコーバック防止。
...
### 以前の問題（解決済み）
- ~~DPIスケール1.25倍速バグ~~ → wintf-fix1/3/4で解決
- ~~thread_local DragStateがwndprocスレッドとECSスレッドで共有されない問題~~ → DragAccumulatorResourceで解決
- ~~ドラッグイベントがとびとびにしか発火しない~~ → 累積器方式で解決
...
- [x] 4.1 apply_window_drag_movement Systemでウィンドウ位置更新
  - apply_window_drag_movement()関数を実装（MessageReader<DragEvent>購読）
  - DragEvent.position と DraggingState.initial_inset から新しいBoxStyle.insetを計算
  - BoxStyle.inset更新→taffyレイアウト→window_pos_sync_system→apply_window_pos_changes パイプラインで自動的にSetWindowPos
  - event.targetの親階層からWindowコンポーネント探索
  - Windowが見つからなければスキップ（将来の非ウィンドウドラッグ対応）
  - DragConfig.move_windowフラグでオプトイン（デフォルト: true）
  - **重要**: BoxStyle.insetを直接更新（物理ピクセル単位）
  - **パイプライン**: BoxStyle.inset → taffy → Arrangement → GlobalArrangement → WindowPos → SetWindowPosCommand
  - **DPI問題**: wintf-fix1/3/4で解決済み。座標系は物理ピクセル統一。
  - _Requirements: 6.1, 6.2, 6.3, 6.6, 13.5_
  - **FILE: crates/wintf/src/ecs/drag/systems.rs (apply_window_drag_movement)**
  - **STATUS: 完了（BoxStyle.inset更新方式で実装。DragConfig.move_windowフラグ対応。）**
...
- [x] 4.4 システムスケジュール統合
  ...
  - **STATUS: 完了（dispatch_drag_events → apply_window_drag_movement → cleanup_drag_state順序で登録済み）**

### Phase 5: マルチモニター対応と高DPI

- [x] 5. マルチモニター座標系とDPI対応
- [x] 5.1 GlobalArrangement仮想スクリーン座標系確認
  ...
  - **DPI問題解決済み**: wintf-fix1/3/4でパイプライン整理完了
  - _Requirements: 8.1, 8.2, 8.4_
  - **STATUS: 完了（コードレビューで全4項目確認: GlobalArrangement.boundsは仮想スクリーン座標、負座標対応済、WM_MOUSEMOVEはWindowPos加算でスクリーン座標変換済、SetWindowPosはi32で仮想スクリーン座標受付）**
...
- [x] 6.1 FlexDemoContainerにドラッグハンドラ登録
  ...
  - **STATUS: 完了。apply_window_drag_movementシステム導入によりハンドラ簡素化済み（ログ出力のみ）。**
...
- [x] 7.1 ドラッグ操作の基本動作確認
  ...
  - ESCキーキャンセルの動作確認
  - WM_ACTIVATE非アクティブ化キャンセルの動作確認（Alt+Tab）
  - ドラッグ閾値（5px）の動作確認
  - 単一クリック（閾値未到達）が正常動作することを確認
  - **追加修正**: hit_test結果の子エンティティから親階層DragConfig探索を実装
  - **追加修正**: WM_ACTIVATEハンドラ追加（Alt+Tabキャンセル対応）
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 2.1, 5.1, 5.2, 5.3_
  - **STATUS: 完了（2026-02-13 手動テスト全項目合格）**
...
- [x] 7.4 パフォーマンス検証
  ...
  - **STATUS: 条件付き合格。Debugビルドでタスクバードラッグ比約「1/3〝のフレームレート低下あり。原因はECSパイプラインレイテンシ（BoxStyle.inset→taffy→WindowPos→SetWindowPos）。将来最適化: WM_MOUSEMOVEで直接SetWindowPos呼出（DragStateにHWND/初期位置キャッシュ）が有効。**

- [x] 7.5 エラーハンドリングとエッジケース検証
  - エンティティ削除時のドラッグ自動キャンセル確認
  - マウスキャプチャ予期しない解放時の動作確認
  - 複数ボタン同時押下時の拒否動作確認
  - フォーカス喪失時のキャンセル動作確認（オプション機能）
  - _Requirements: 1.7, 5.4, 5.5, 5.6, 14.4_
  - **STATUS: 完了（2026-02-13 手動テスト。右ボタン同時押下、ドラッグ中右クリック、長時間ドラッグすべてクラッシュなし）**
