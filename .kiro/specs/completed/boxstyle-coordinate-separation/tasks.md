...
- [x] 1. ECS インフラ構築
- [x] 1.1 (P) WindowDragContextResource の追加
...
- [x] 1.2 (P) WindowDragging マーカーコンポーネントの追加
...
- [x] 2. WM_WINDOWPOSCHANGED ハンドラの変更
- [x] 2.1 (P) サイズ変更判定ロジックの実装
...
- [x] 2.2 (P) BoxStyle.inset 書き込みの除去
...
- [x] 3. ドラッグシステムの WndProc レベル移行
- [x] 3.1 DragState enum の拡張
...
- [x] 3.2 dispatch_drag_events の変更
...
- [x] 3.3 (P) apply_window_drag_movement システムの削除
...
- [x] 4. WM_MOUSEMOVE ハンドラでの直接 SetWindowPos 実装
- [x] 4.1 Dragging 状態での WndProc レベルウィンドウ移動の実装
...
- [x] 5. update_arrangements_system の変更
- [x] 5.1 (P) Window entity の offset スキップロジックの追加
...
- [x] 6. サンプルアプリケーションの初期位置指定変更
- [x] 6.1 Examples の初期位置を WindowPos に移行
...
- [x] 7. 統合テストおよび E2E テストの実装
- [x] 7.1* BoxStyle.inset 不変性テスト
  - WM_WINDOWPOSCHANGED シミュレーション後に Window entity の BoxStyle.inset が変更されていないことを検証
  - _Requirements: 1.1, 1.5_

- [x] 7.2* Changed<BoxStyle> 発火タイミングテスト
