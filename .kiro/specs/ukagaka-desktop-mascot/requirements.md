...
以下はプラットフォームの責務外として明示的にスコープ外とする：

1. **アクセシビリティ対応**: ゴースト/シェル/バルーンの表現（色彩、フォント、音声等）は**コンテンツ作者の責務**。プラットフォームが作者の意図した表現を勝手に変更することは、創作の尊重の観点から不適切と判断する。（例：色弱者向けの色調補正をシェルに強制適用することはNG）
2. **コンテンツの国際化**: ゴースト/シェル/バルーンの多言語対応は**作者の責務**。プラットフォームはコンテンツの言語に関与しない。LLM利用ゴーストは自動的に多言語対応可能、バルーンはユーザーが別バルーンを選択して対応。

> **注記**: プラットフォーム提供UIの国際化はNFR-5で対応。セキュリティに関わる制約はNFR-3で対応済み。
...
本要件定義書における「実装」とは、**子仕様（詳細設計書）の作成**を指す。本仕様を直接プログラムコードに落とし込むことは現実的ではなく、各要件に対応する詳細設計を段階的に作成することで実現を目指す。
...
画面に現れた彼女が、声で迎えてくれる。VOICEVOX。2000年には存在しなかった、高品質な音声合成。
...
ポータブル設計——フォルダ丸ごとコピーで、彼女との日々も一緒に移行できる。
...
かつて、利用規約をパロディにした自己紹介で登場したゴーストがいた。真面目な体裁を自分でツッコみ、相方との掛け合いで本音を見せる——そんなスタイルが、多くの人の心を掴んだ。

2025年のファーストゴーストは、どう自己紹介するだろう？
...
「頭は撫でていい。それ以外は...まあ、自己責任で」

「...なんか保護者みたいになった」「気にしないで」
