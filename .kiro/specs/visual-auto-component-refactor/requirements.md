...
3. **重要:** ウィンドウのルートVisualとなるエンティティについては、自動生成された `VisualGraphics` を `WindowGraphics` (CompositionTarget) のルートとして設定する処理 (`SetRoot`) を確実に移行・維持しなければならない。この紐付けが欠落すると画面描画が行われないため、最優先で検証する。
4. リファクタリング前後で、アプリケーションの描画結果や挙動に変化がないこと（リグレッションがないこと）を保証しなければならない。
