...
4. **The** Dola **shall** エントリに以下のフィールドを定義できる：
   ...
   - `at`（開始キーフレーム指定。文字列（単一キーフレーム名）または配列（複数キーフレーム名）を受け付ける。配列で複数指定時は全キーフレーム完了待機。オプションで `offset`（f64秒）を付与可）
...
5. **The** Dola **shall** エントリの配置方法として以下の3種類をサポートする：
   ...
   - **キーフレーム起点**: `at` で開始キーフレームを指定。文字列で単一指定、または配列で複数指定が可能。配列で複数指定した場合は、指定された全キーフレームが完了した時点（最遅）を開始基準とする（WAM `AddTransitionAtKeyframe` の拡張）
...
9. **The** Dola **shall** 各ストーリーボードに割り込み終了戦略（`interruption_policy`）を定義できる（他のストーリーボードとの競合時に、このストーリーボードをどのように終了させるかをストーリーボード自身が宣言する。Cancel/Conclude/Trim/Compress/Never から選択。デフォルトは Conclude。マルチプロセス協調アニメーション環境において、各ストーリーボードは自身の終了方針を自己申告し、オーケストレーション側の解決ロジックに情報提供する）
