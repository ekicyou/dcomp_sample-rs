...
4. **The** Dola **shall** エントリに以下のフィールドを定義できる：
   ...
   - `at`（開始キーフレーム、キーフレーム名の配列。複数指定時は全キーフレーム完了待機。オプションで `offset`（f64秒）を付与可）
...
5. **The** Dola **shall** エントリの配置方法として以下の3種類をサポートする：
   ...
   - **キーフレーム起点**: `at` で開始キーフレームを指定。配列で複数指定した場合は、指定された全キーフレームが完了した時点（最遅）を開始基準とする（WAM `AddTransitionAtKeyframe` の拡張）
