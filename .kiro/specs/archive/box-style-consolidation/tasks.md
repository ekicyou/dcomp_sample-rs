...
- [x] 1. BoxStyle統合コンポーネントの実装
- [x] 1.1 (P) BoxStyle構造体を定義し、全レイアウトプロパティを統合する
...
- [x] 1.2 (P) 既存のBox系・Flex系コンポーネントからComponent deriveを削除し、値オブジェクト化する
...
- [x] 1.3 BoxStyleからtaffy::Styleへの変換トレイトを実装する
...
- [x] 2. レイアウトシステムのクエリ簡略化
- [x] 2.1 build_taffy_styles_systemを新しいBoxStyle単一クエリに更新する
...
- [x] 3.2 (P) サンプルアプリケーションを新APIに移行する
  - taffy_flex_demo.rsのコンポーネント使用箇所をBoxStyleに変更
  - 動作確認（表示結果が現行と同一であること）
  - _Requirements: 3.4_

- [x] 4. ビルド検証と最終確認
- [x] 4.1 全ターゲットビルドとテスト実行で移行漏れがないことを確認する
