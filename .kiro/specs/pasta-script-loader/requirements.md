...
#### Acceptance Criteria
1. The Pastaエンジン shall 全ファイルのグローバルラベルをグローバル名前空間へ登録する
2. The Pastaエンジン shall 各ファイル内のローカルラベルをファイルスコープ内でのみアクセス可能とする
3. When 異なるファイルで同名のローカルラベルが定義されている場合、the Pastaエンジン shall それぞれを独立したラベルとして扱う
4. When 開発者がラベルを実行する場合、the Pastaエンジン shall グローバルラベルを優先的に検索する
5. If グローバル名前空間に該当ラベルが存在しない場合、then the Pastaエンジン shall カレントファイルスコープのローカルラベルを検索する
