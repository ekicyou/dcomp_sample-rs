...
- [ ] 1. RemoveVisual APIラッパーの実装
- [ ] 1.1 (P) DCompositionVisualExtトレイトにremove_visual/remove_all_visualsメソッドを追加
  - DirectComposition RemoveVisual APIを呼び出すラッパーメソッドを実装
  - RemoveAllVisuals APIのラッパーも併せて実装
  - 存在しないVisualの削除時のエラーハンドリングを実装
  - _Requirements: R1_

- [ ] 2. VisualGraphicsの親キャッシュ拡張
- [ ] 2.1 VisualGraphicsコンポーネントにparent_visualフィールドを追加
...
- [ ] 3. Visual追加ヘルパー関数の実装
- [ ] 3.1 (P) insert_visual/insert_visual_with関数をecs::graphicsモジュールに公開
...
- [ ] 4. Visual追加時のVisualGraphics自動作成
- [ ] 4.1 Added<Visual>を検知してVisualGraphicsを作成するシステムを実装
...
- [ ] 5. 既存ウィジェットへのVisual自動追加
- [ ] 5.1 (P) LabelコンポーネントにVisual自動追加フックを実装
  - on_addフックでinsert_visualを呼び出し
  - _Requirements: R4_

- [ ] 5.2 (P) RectangleコンポーネントにVisual自動追加フックを実装
  - on_addフックでinsert_visualを呼び出し
  - _Requirements: R4_

- [ ] 6. ウィジェットツリー変更の検知システム
- [ ] 6.1 ChildOf変更を検知してVisual階層を同期するシステムを実装
...
- [ ] 6.2 Children順序変更を検知してZ-orderを同期するシステムを実装
  - Changed<Children>で順序変更を検知
  - AddVisualのinsertabove/referencevisualパラメータでZ-order制御
  - _Requirements: R7_
...
- [ ] 11.1 world.rsに新規システムを登録
  ...
  - RenderSurface: render_to_surface_system（Changed<GraphicsCommandList>フィルタ）
  - _Requirements: R10_

## Phase 6: 統合テストと検証

- [ ] 12. 統合テストの実装
- [ ] 12.1 階層追加/削除シナリオのテストを実装
  - ChildOf追加時のVisual階層同期を検証
  - ChildOf変更時の旧親→新親移動を検証
  - Entity despawn時のVisualクリーンアップを検証
  - _Requirements: R6, R7, R9_

- [ ] 12.2 Z-order変更シナリオのテストを実装
  - Children順序変更時のZ-order同期を検証
  - _Requirements: R7_

- [ ] 12.3 dcomp_demo.rsでビジュアル回帰テストを実行
