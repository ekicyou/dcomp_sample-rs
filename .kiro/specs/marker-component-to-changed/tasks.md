...
- [ ] 1. SurfaceGraphicsDirty コンポーネント定義
- [ ] 1.1 (P) SurfaceGraphicsDirty コンポーネントを追加
...
- [ ] 2. HasGraphicsResources 拡張
- [ ] 2.1 (P) HasGraphicsResources に世代番号フィールドとメソッドを追加
...
- [ ] 2.2 init_graphics_core システムを Changed パターンに変更
  - `insert(GraphicsNeedsInit)` を `res.request_init()` に置換
  - 再初期化時と初期化時の両方の呼び出し箇所を変更
  - _Requirements: 4.1_

- [ ] 2.3 init_window_graphics システムを Changed パターンに変更
  - `With<GraphicsNeedsInit>` を `Changed<HasGraphicsResources>` に置換
  - `res.needs_init()` 条件チェックを追加
  - _Requirements: 4.2_

- [ ] 2.4 init_window_visual システムを Changed パターンに変更
  - `With<GraphicsNeedsInit>` を `Changed<HasGraphicsResources>` に置換
  - `res.needs_init()` 条件チェックを追加
  - _Requirements: 4.3_

- [ ] 2.5 cleanup_graphics_needs_init システムを Changed パターンに変更
  - `With<GraphicsNeedsInit>` を `needs_init()` 条件に置換
  - `remove::<GraphicsNeedsInit>()` を `res.mark_initialized()` に置換
  - _Requirements: 4.4_

- [ ] 2.6 cleanup_command_list_on_reinit システムを Changed パターンに変更
  - `With<GraphicsNeedsInit>` を `needs_init()` 条件に置換
  - _Requirements: 4.5_

- [ ] 2.7 create_visuals_for_init_marked システムを Changed パターンに変更
  - `With<GraphicsNeedsInit>` を `Changed<HasGraphicsResources>` + `needs_init()` に置換
  - _Requirements: 4.6_

- [ ] 2.8 GraphicsNeedsInit コンポーネントを削除
...
- [ ] 3. テスト更新と統合検証
- [ ] 3.1 (P) HasGraphicsResources メソッドのユニットテストを追加
...
- [ ] 3.2 surface_optimization_test.rs のテストを更新
