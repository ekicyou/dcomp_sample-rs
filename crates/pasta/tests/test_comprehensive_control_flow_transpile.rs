...
fn test_comprehensive_control_flow_transpile() {
    ...
    // 基本的な構造チェック
    assert!(rune_code.contains("pub mod メイン_1"), "グローバルラベルのモジュールが生成されていない");
    assert!(rune_code.contains("pub fn __start__(ctx, args)"), "__start__関数が生成されていない");
    assert!(rune_code.contains("pub fn 自己紹介_1(ctx, args)"), "ローカルラベル関数が生成されていない");
    assert!(rune_code.contains("mod pasta"), "mod pastaが生成されていない");
    assert!(rune_code.contains("pub fn call("), "pasta::call関数が生成されていない");
    assert!(rune_code.contains("pub fn jump("), "pasta::jump関数が生成されていない");
    
    // Runeブロックがそのまま含まれているか確認
    assert!(rune_code.contains("pub fn ローカル関数(ctx, args)"), "Runeブロックが含まれていない");
...
fn test_comprehensive_control_flow_rune_compile() {
    ...
    context.install(stdlib::create_module().expect("Failed to create pasta_stdlib module"))
...
"#;
    ...
    sources.insert(Source::new("main", main_rn).expect("Failed to create main source"))
        .expect("Failed to insert main source");
    sources.insert(Source::new("entry", &rune_code).expect("Failed to create source"))
    ...
    assert!(!diagnostics.has_error(), "Runeコンパイルでエラーが発生: {:?}", diagnostics);
