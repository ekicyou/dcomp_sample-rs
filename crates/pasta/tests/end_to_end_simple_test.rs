...
"#;
    ...
    context.install(pasta::stdlib::create_module().expect("Failed to create stdlib")).expect("Failed to install stdlib");
    
    let mut sources = Sources::new();
    sources.insert(rune::Source::new("entry", &rune_code).expect("Failed to create source")).expect("Failed to add source");
    ...
    // Execute
    let mut vm = Vm::new(std::sync::Arc::new(context.runtime().expect("Failed to get runtime")), std::sync::Arc::new(unit));
...
"#;
    ...
    ctx_obj.insert(rune::alloc::String::try_from("actor").unwrap(), rune::to_value("").unwrap()).unwrap();
    let ctx = rune::to_value(ctx_obj).expect("Failed to create context");
    
    // Execute as generator - vm.call returns Value, need to convert
    let value = vm.call(["会話_1", "__start__"], (ctx,)).expect("Failed to call");
    let generator = rune::from_value::<rune::runtime::Generator<_>>(value).expect("Failed to convert to generator");
