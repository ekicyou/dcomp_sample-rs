...
fn test_comprehensive_control_flow_transpile() {
    ...
    // Basic validations
    assert!(
        transpiled.contains("pub mod メイン_1"),
        "Should have メイン_1 module"
    );
    assert!(
        transpiled.contains("pub fn __start__(ctx)"),
        "Should have __start__ function"
    );
    assert!(
        transpiled.contains("pub mod pasta"),
        "Should have pasta module"
    );
    assert!(
        transpiled.contains("pub fn jump"),
        "Should have jump function"
    );
    assert!(
        transpiled.contains("pub fn call"),
        "Should have call function"
    );
    assert!(
        transpiled.contains("pub fn label_selector"),
        "Should have label_selector function"
    );

    // Check for control flow statements
    assert!(
        transpiled.contains("pasta::call"),
        "Should contain call statements"
    );
    assert!(
        transpiled.contains("pasta::jump"),
        "Should contain jump statements"
    );
...
fn test_comprehensive_control_flow_has_local_labels() {
    ...
    let ast = parse_str(&pasta_code, "comprehensive_control_flow.pasta").expect("Failed to parse");
    ...
    assert!(
        transpiled.contains("pub fn 自己紹介"),
        "Should have 自己紹介 local label"
    );
    assert!(
        transpiled.contains("pub fn 趣味紹介"),
        "Should have 趣味紹介 local label"
    );
    assert!(
        transpiled.contains("pub fn カウント表示"),
        "Should have カウント表示 local label"
    );
    assert!(
        transpiled.contains("pub fn 会話分岐"),
        "Should have 会話分岐 local label"
    );
    assert!(
        transpiled.contains("pub fn 別の話題"),
        "Should have 別の話題 local label"
    );
...
fn test_comprehensive_control_flow_actor_extraction() {
    ...
    assert!(
        transpiled.contains("ctx.actor = さくら"),
        "Should have さくら actor assignment"
    );
    assert!(
        transpiled.contains("ctx.actor = うにゅう"),
        "Should have うにゅう actor assignment"
    );
