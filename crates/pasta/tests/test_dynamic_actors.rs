...
"#;
    ...
    assert!(transpiled.contains("use crate::{さくら};"), "Should import only さくら");
    assert!(!transpiled.contains("うにゅう"), "Should not import うにゅう");
}
...
"#;
    ...
    assert!(transpiled.contains("use crate::{"), "Should have use crate import");
...
"#;
    ...
    assert!(transpiled.contains("さくら"), "Should import さくら from 会話1");
    assert!(transpiled.contains("うにゅう"), "Should import うにゅう from 会話2");
    assert!(transpiled.contains("みつこ"), "Should import みつこ from 会話3");
    assert!(transpiled.contains("ななこ"), "Should import ななこ from 会話3");
}
...
"#;
    ...
    assert_eq!(import_count, 2, "Should have exactly 2 modules with さくら import (会話1_1 and 会話2_1)");
    ...
    for line in import_lines {
        ...
        assert_eq!(line.matches("さくら").count(), 1, "Should have さくら only once per import line");
    }
}
