...
fn test_transpile_context_default_global_functions() {
    ...
    assert!(context.resolve_function("emit_text", FunctionScope::Auto).is_ok());
    assert!(context.resolve_function("change_speaker", FunctionScope::Auto).is_ok());
    assert!(context.resolve_function("wait", FunctionScope::Auto).is_ok());
}
...
fn test_multiple_local_functions() {
    ...
    assert!(context.resolve_function("func1", FunctionScope::Auto).is_ok());
    assert!(context.resolve_function("func2", FunctionScope::Auto).is_ok());
    assert!(context.resolve_function("func3", FunctionScope::Auto).is_ok());
    
    // None should be resolvable with GlobalOnly scope
    assert!(context.resolve_function("func1", FunctionScope::GlobalOnly).is_err());
    assert!(context.resolve_function("func2", FunctionScope::GlobalOnly).is_err());
    assert!(context.resolve_function("func3", FunctionScope::GlobalOnly).is_err());
}
