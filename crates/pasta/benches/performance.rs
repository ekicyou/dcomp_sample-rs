...
fn generate_large_script(num_labels: usize, lines_per_label: usize) -> String {
    ...
    for i in 0..num_labels {
        ...
        for j in 0..lines_per_label {
            let speaker = if j % 2 == 0 { "さくら" } else { "うにゅう" };
            script.push_str(&format!("    {}：これは{}行目のテキストです。\n", speaker, j + 1));
        }
...
"#;
    ...
    for i in 0..100 {
        ...
        let events = engine.execute_label(label).expect("Failed to execute label");
...
fn benchmark_label_lookup() {
    ...
    for i in 0..1000 {
        ...
        assert!(engine.has_label(&label_name), "Label not found: {}", label_name);
    }
    let duration = start.elapsed();
...
fn benchmark_duplicate_labels() {
    ...
    let start = Instant::now();
    for _ in 0..100 {
        let events = engine.execute_label("挨拶").expect("Failed to execute label");
        assert!(!events.is_empty(), "No events generated");
    }
    let duration = start.elapsed();
...
"#;
    ...
    while start.elapsed() < duration_limit {
        let events = engine.execute_label("test").expect("Failed to execute label");
    ...
    println!("Events per second: {:.0}", total_events as f64 / duration.as_secs_f64());
