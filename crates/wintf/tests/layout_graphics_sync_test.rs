...
fn test_visual_partial_eq_optimization() {
    ...
    let visual3 = Visual {
        size: windows_numerics::Vector2 { X: 1024.0, Y: 768.0 },
        ..Default::default()
    };
    
    // 同じ値を持つVisualは等しいべき
    assert_eq!(visual1, visual2, "同じ値を持つVisualはPartialEqでtrueを返すべき");
    
    // 異なる値を持つVisualは等しくないべき
    assert_ne!(visual1, visual3, "異なる値を持つVisualはPartialEqでfalseを返すべき");
